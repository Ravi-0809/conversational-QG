...
import onmt.modules
from onmt.encoders.memory_encoder import MemoryEncoder
from onmt.decoders.memory_decoder import MemoryDecoder
from onmt.encoders.rnn_encoder import InputFeedRNNEncoder
from onmt.decoders.rnn_decoder import InputFeedRNNDecoder
from onmt.encoders.query_encoder import QueryEncoder
from onmt.decoders.query_decoder import QueryDecoder

from onmt.modules import Embeddings, CopyGenerator, CorefGenerator
...
def build_encoder(opt, embeddings):
    ...
    """
    return MemoryEncoder(opt.rnn_type, opt.encoder_type,
                         opt.passage_enc_layers,
                         opt.qa_word_enc_layers,
                         opt.qa_sent_enc_layers,
                         opt.enc_rnn_size, opt.dropout,
                         embeddings, opt.self_attention)
...
def build_decoder(opt, embeddings):
    ...
    """
    # here we use sentence last state, so we set brnn false
    opt.brnn = False
    return MemoryDecoder(opt.rnn_type, opt.brnn, opt.dec_layers,
