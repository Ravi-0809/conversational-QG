...
class Translator(object):
    ...
    def translate(self,
        ...
        #             print(msg)
        #         if self.report_bleu:
        #             msg = self._report_bleu(data_path)
        #             if self.logger:
        #                 self.logger.info(msg)
        #             else:
        #                 print(msg)
        #         if self.report_rouge:
        #             msg = self._report_rouge(data_path)
        #             if self.logger:
        #                 self.logger.info(msg)
        #             else:
        #                 print(msg)
    ...
    def translate_batch(self, batch, data):
        ...
        batch.src_map = batch.map  # for fear of bugs
        ...
        src_bank, qa_sent_bank, qa_word_bank, src_state = \
            self.model.encoder(src, src_lengths, qa, qa_sent_lengths, qa_word_lengths)
        dec_states = self.model.decoder.init_decoder_state(src_state)

        # (2) Repeat src objects `beam_size` times.
        src_map = rvar(batch.map.data) if self.copy_attn else None

        src_bank = rvar(src_bank.data)
        src_lengths = src_lengths.repeat(beam_size)
        qa_word_bank = var(qa_word_bank.data.repeat(1, beam_size, 1, 1))
        qa_word_lengths = qa_word_lengths.repeat(beam_size, 1)
        qa_sent_bank = rvar(qa_sent_bank.data)
        qa_sent_lengths = qa_sent_lengths.repeat(beam_size)
    ...
    #     mem_ans, mem_ans_turn, mem_ans_length = batch.memory_answer[0], batch.memory_answer[1], \
    #                                             batch.memory_answer[2]
    #     mem_que, mem_que_turn, mem_que_length = batch.memory_question[0], batch.memory_question[1], \
    #                                             batch.memory_question[2]
    #     cur_ans, cur_ans_length = batch.current_answer[0], batch.current_answer[1]
    #     tgt_in = batch.tgt[:-1]
    #
    #     #  (1) run the encoder on the src
    #     mem_rtl_bank, mem_rtl_state, mem_ans_bank, _, \
    #     cur_ans_bank, _, mem_que_bank, _ = self.model.encoder(mem_rtl, mem_rtl_length,
    #                                                     mem_ans, mem_ans_turn, mem_ans_length,
    #                                                     mem_que, mem_que_turn, mem_que_length,
    #                                                     cur_ans, cur_ans_length)
    #     dec_states = \
    #         self.model.decoder.init_decoder_state(mem_rtl_state)
    #
    #     #  (2) if a target is specified, compute the 'goldScore'
    #     #  (i.e. log likelihood) of the target under the model
    #     tt = torch.cuda if self.cuda else torch
    #     gold_scores = tt.FloatTensor(batch.batch_size).fill_(0)
    #     dec_out, _, _ = self.model.decoder(tgt_in, mem_rtl_bank, mem_ans_bank, cur_ans_bank, mem_que_bank,
    #                                        dec_states, mem_rtl_length, mem_ans_turn, mem_ans_length,
    #                                        mem_que_turn, mem_que_length, cur_ans_length)
    #
